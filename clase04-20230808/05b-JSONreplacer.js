...
let path='./archivos/heroes.json'

// fs.writeFileSync(path,JSON.stringify(heroes,['name','picture'],'\t'))

function modifica(clave, valor){
    if(clave==='name'){
        return {
            nombre:valor.toUpperCase(),
            nombre_original:valor,
            codigo:'001',
            // name: 'Pirulo'
        }
    }
    ...
    if(clave==='codigo'){
        return valor+' - esto lo agregue recursivamente...'
    }

    return valor
}

fs.writeFileSync(path,JSON.stringify(heroes,modifica,'\t'))
